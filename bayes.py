...
class BayesABTestABC(ABTestABC, metaclass=abc.ABCMeta):
    ...
    def _compute_posterior_analytical(self, *args, **kwargs):
        raise RuntimeError("No analytical solution is available. Use the numerical one.")
    ...
    def plot_expected_loss(
        ...
        for e in experiments_idxs:
            for i, (ela, elb, ila, ilb) in enumerate(arr):
                plt.plot(range(len(ela[e])), ela[e], color=colour_1, linestyle=linestyles[i])
                plt.plot(range(len(ela[e])), elb[e], color=colour_2, linestyle=linestyles[i])
                if plot_interval:
                    plt.plot(range(len(ila[e, :, 0])), ila[e, :, 0], color=colour_1, linestyle=":")
                    plt.plot(range(len(ila[e, :, 1])), ila[e, :, 1], color=colour_1, linestyle=":")
                    plt.plot(range(len(ilb[e, :, 0])), ilb[e, :, 0], color=colour_2, linestyle=":")
                    plt.plot(range(len(ilb[e, :, 1])), ilb[e, :, 1], color=colour_2, linestyle=":")

        plt.hlines([threshold], 0, len(ela[e]), linestyle="--")
        plt.xlabel("n days")
        plt.ylabel("exp loss")
        blue_patch = matplotlib.patches.Patch(color="blue", label=f"E[L({label_1})]")
        orange_patch = matplotlib.patches.Patch(color="orange", label=f"E[L({label_2})]")
        ...
        a = np.array(
            list(
                map(
                    ...
                    & ((1.0 - p_b_over_a[:, initial_days:]) >= probbaility_threashold_for_being_better),
                )
            )
        )
        b = np.array(
            list(
                map(
                    ...
                    & (p_b_over_a[:, initial_days:] >= probbaility_threashold_for_being_better),
                )
            )
        )
    ...
    def produce_table_results(self, product, days_for_significance=3, **kwargs) -> pd.DataFrame:
        ...
        expected_loss_a, expected_loss_b, interval_loss_a, interval_loss_b, p_b_over_a = self.run_test(exact=True)
...
class BayesianInferenceMixin:
    ...
    def _run_mcmc(self, init_state, target_log_prob_fn: Callable, mcmc_samples: int, n_chains: int = 1):
        ...
        kernel = tfp.mcmc.DualAveragingStepSizeAdaptation(
            tfp.mcmc.TransformedTransitionKernel(
                inner_kernel=tfp.mcmc.NoUTurnSampler(target_log_prob_fn, step_size=[step_size]), bijector=bijectors_list
            ),
...
class ConversionRateBayesABTest(BayesianInferenceMixin, BayesABTestABC):
    def __init__(
        self, *, prior_a: Optional[tfd.Distribution] = None, prior_b: Optional[tfd.Distribution] = None, **kwargs
    ):
    ...
    def _compute_posterior_numerical(self, conversions, counts, prior: tfd.Distribution, mcmc_samples: int = 10_000):
        ...
        model = tfd.JointDistributionSequential(
            [
                ...
                lambda p: tfd.Sample(
                    tfd.Binomial(tf.convert_to_tensor(counts.ravel(), dtype=tf.float32), probs=p),
                ),
            ]
        )
        n_chains = 3
        init_state = np.tile(model.sample()[0].numpy(), (n_chains, np.prod(counts.shape)))
        target_log_prob_fn = lambda p: model.log_prob(p, conversions.ravel())
        posterior_samples = self._run_mcmc(init_state, target_log_prob_fn, mcmc_samples=mcmc_samples, n_chains=n_chains)
        return model, posterior_samples[0][0].numpy().mean(axis=1).reshape(-1, *counts.shape)

    def _compute_posterior_analytical(self, conversions, counts, prior, mcmc_samples: int = 10_000):
    ...
    def _compute_expected_loss(
        ...
        expected_loss_a, interval_loss_a, expected_loss_b, interval_loss_b, prob_b_vs_a = (
        ...
        for e in tqdm.tqdm(range(n_experiments)):
            for d in range(n_days):
                ...
                # (2) This way of doing it we include the cases where the loss is negative in the pdf:
                interval_loss_b[e, d] = az.hdi(-d_pdf * np.heaviside(-d_pdf, 0), 0.95)  # this is

                prob_b_vs_a[e, d] = (posterior_b_samples[:, e, d] > posterior_a_samples[:, e, d]).astype(
                    int
                ).sum() / mcmc_samples
        return expected_loss_a, expected_loss_b, interval_loss_a, interval_loss_b, prob_b_vs_a

    def run_test(self, exact: bool = False) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        posterior_samples_fn = self._compute_posterior_numerical
        if exact is True:
            posterior_samples_fn = self._compute_posterior_analytical

        (conversions_a, counts_a), (conversions_b, counts_b) = self.data_loader.load_data()
        posterior, posterior_a_samples = posterior_samples_fn(conversions_a, counts_a, self.prior_a)
        posterior, posterior_b_samples = posterior_samples_fn(conversions_b, counts_b, self.prior_b)
    ...
    def _compare_numerical_and_analyitical_solution(self, plot: bool = True):
        (conversions_a, counts_a), (conversions_b, counts_b) = self.data_loader.load_data()
        _, posterior_a_samples = self._compute_posterior_numerical(conversions_a, counts_a, self.prior_a)
        _, posterior_b_samples = self._compute_posterior_numerical(conversions_b, counts_b, self.prior_b)
        losses_numerical = self._compute_expected_loss(posterior_a_samples, posterior_b_samples)
        _, pa = self._compute_posterior_analytical(conversions_a, counts_a, self.prior_a, 10_000)
        _, pb = self._compute_posterior_analytical(conversions_b, counts_b, self.prior_b, 10_000)
        losses_analytical = self._compute_expected_loss(pa, pb)
        if plot:
            self.plot_expected_loss([losses_numerical[:2], losses_analytical[:2]], experiments_idxs=[0])
...
class RevenuePerUserBayesABTest(BayesianInferenceMixin, BayesABTestABC):
    ...
    def _compute_posterior_analytical(self, conversions, revenues, priors, mcmc_samples):
        logger.info("Computing the posterior for the conversion rates (analytical) ...")
        ...
        if len(rpu[rpu < 0.0]):
            logger.warning(f"There are some full days with negative revenues: {rpu[rpu < 0.]}. Will set them to 0.0")
            rpu[rpu < 0.0] = 0.0
        ...
        return (posterior_conversion, posterior_rev), posterior_samples_conversion / posterior_samples_rev
    ...
    def _compute_expected_loss(
        ...
        loss_b = np.maximum(np.abs(posterior_a_samples) - np.abs(posterior_b_samples), 0)
        loss_a = np.maximum(np.abs(posterior_b_samples) - np.abs(posterior_a_samples), 0)
        ...
        interval_loss_a = np.moveaxis(np.apply_along_axis(lambda x: az.hdi(x, 0.95), axis=0, arr=b_vs_a_loss_a), 0, -1)
        ...
        interval_loss_b = np.moveaxis(np.apply_along_axis(lambda x: az.hdi(x, 0.95), axis=0, arr=b_vs_a_loss_b), 0, -1)
    ...
    def run_test(self, exact: bool = False) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        ...
        _, posterior_a_samples = posterior_samples_fn(
            conversions_a, revenues_a, {"p": self.prior_pa, "theta": self.prior_revenues_on_sale}, mcmc_samples=10_000
        )
        _, posterior_b_samples = posterior_samples_fn(
            conversions_b, revenues_b, {"p": self.prior_pb, "theta": self.prior_revenues_on_sale}, mcmc_samples=10_000
        )
        return self._compute_expected_loss(posterior_a_samples, posterior_b_samples)
